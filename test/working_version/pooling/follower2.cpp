...
void setup() {
  ...
  Serial.println("Follower2: CAN bus initialized.");
}
...
void loop() {
    ...
    for (int i = 0; i < 6; i++) {
      outgoingMsg.buf[i] = 30 - i * 5;  // Data: 10, 20, 30, 40, 50, 60
    }
  
    Serial.print("Follower2: Sending message with ID 0x");
    Serial.println(outgoingMsg.id, HEX);
    int ret = Can.write(MB1, outgoingMsg);
    if(ret == 1) {
      Serial.println("Follower2: Message sent successfully.");
      Serial.print("Data: ");
      ...
      Serial.print("Follower2: Error sending message. Error code: ");
      Serial.println(ret);
    }
    ...
    if(r == 1) {
      Serial.print("Follower2: Received message with ID 0x");
      ...
      Serial.println("Follower2: No readable message available.");
    }
